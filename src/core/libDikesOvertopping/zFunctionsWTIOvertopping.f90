...
module zFunctionsWTIOvertopping
    ...
    use typeDefinitionsRTOovertopping,  only : xDiff_min, slope_min, tpGeometry, tpLoad, tpModelFactors, tpOvertopping, OvertoppingModelFactors
    use mainModuleRTOovertopping,       only : calculateOvertopping, fillModelfactorsRTO, convertModelfactorsRTO
...
!! @ingroup LSF
subroutine calculateQoRTO( dikeHeight, modelFactorsC, overtopping, load, geometry, succes, errorMessage )
    real(kind=wp),                 intent(in)  :: dikeHeight     !< dike height
    type(OvertoppingModelFactors), intent(in)  :: modelFactorsC  !< struct with model factors
    ...
    if (succes) then
        ...
        if (succes) then
            ...
            call convertModelfactorsRTO(modelFactors, modelFactorsC)

            call calculateOvertopping (geometryAdjusted, load, modelFactors, .true., overtopping, succes, errorMessage)
...
!! @ingroup LSF
subroutine profileInStructure( nrCoordinates, xcoordinates, ycoordinates, dikeHeight, nrCoordsAdjusted, xCoordsAdjusted, zCoordsAdjusted, succes, errorMessage )
