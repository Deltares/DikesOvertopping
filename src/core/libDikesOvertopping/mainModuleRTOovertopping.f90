...
!***********************************************************************************************************
   ...
   public :: calculateOvertopping, calculateOvertoppingSection, iterationWaveRunup, calculateWaveOvertopping
   public :: interpolateResultsSections, checkInputdata, fillModelfactorsRTO, convertModelfactorsRTO
...
!***********************************************************************************************************
   subroutine calculateOvertopping (geometry, load, modelFactors, probContext, overtopping, succes, errorMessage)
!***********************************************************************************************************
!
   implicit none
!
...
!
   ...
   type (tpModelFactors),  intent(in)  :: modelFactors   !< structure with model factors
   logical,                intent(in)  :: probContext    !< flag for using the overtopping module in a probabilistic context 
...
! ==========================================================================================================
   ...
   call checkInputdata (geometry, load, modelFactors, probContext, succes, errorMessage)
   ...
   if (succes) then
      ...
      Hm0   = min(load%Hm0, max((load%h - toe)/2.0d0, 0.0d0))
      Tm_10 = load%Tm_10
      ...
      if (isEqualZero (Hm0) .or. isEqualZero (Tm_10)) then
      ...
      if (load%h > crest) then
         !call calculateOvertoppingNegativeFreeboard (load, geometry, overtopping, succes, errorMessage)
      endif
   endif
...
! ==========================================================================================================
   ...
   select case (foreshoreCase)
      ...
      ! ----------------------
         ...
         call iterationWaveRunup (geometry, h, Hm0, Tm_10, L0, gammaBeta_z, &
      ...
      ! -----------------------------------------------
         ...
         Hm0_red = min(Hm0,dH/2)

         ! calculate 2% wave run-up
         if (succes) then
            call iterationWaveRunup (geometryAdjusted, h, Hm0_red, Tm_10, L0, gammaBeta_z, &
                                     modelFactors, overtopping%z2, succes, errorMessage)
         endif
...
!!   @ingroup LibOvertopping
!***********************************************************************************************************
   subroutine iterationWaveRunup (geometry, h, Hm0, Tm_10, L0, gammaBeta_z, modelFactors, z2, &
                                  succes, errorMessage)
!***********************************************************************************************************
!
   implicit none
...
!
   ...
   real(wp)          :: z2_start  (z2_iter_max) !< starting value 2% wave run-up for each iteration step
   real(wp)          :: tanAlpha  (z2_iter_max) !< representative slope angle    for each iteration step
   real(wp)          :: ksi0      (z2_iter_max) !< breaker parameter             for each iteration step
   real(wp)          :: ksi0Limit (z2_iter_max) !< limit value breaker parameter for each iteration step
   real(wp)          :: z2_smooth (z2_iter_max) !< 2% wave run-up (no roughness) for each iteration step
   real(wp)          :: gammaF    (z2_iter_max) !< influence factor roughness    for each iteration step
   real(wp)          :: z2_rough  (z2_iter_max) !< 2% wave run-up (no berms)     for each iteration step
   real(wp)          :: gammaB    (z2_iter_max) !< influence factor berms        for each iteration step
   real(wp)          :: z2_end    (z2_iter_max) !< ending value 2% wave run-up   for each iteration step
...
! ==========================================================================================================
   ...
   do i=1, z2_iter_max
      ...
      if (i == 1) then
         z2_start(i) = 1.5d0*Hm0
      else
         z2_start(i) = z2_end(i-1)
      endif
      ...
      if (succes) then
         if (z2_start(i) > 0.0d0) then
            call calculateWaveRunup (Hm0, s0, ksi0(i), ksi0Limit(i), gammaB(i), gammaF(i), &
   ...
   if (succes) then
      ...
      if (convergence) then
         ...
         ! when no convergence is reached, check if 2% wave run-up 'jumps' between two values
         if ((abs(z2_end(Niterations  ) - z2_end(Niterations-2)) < z2_margin) .and. &
             (abs(z2_end(Niterations-1) - z2_end(Niterations-3)) < z2_margin)) then

            ! in this case 2% wave run-up is the average of these values
            z2 = (z2_end(Niterations) + z2_end(Niterations-1))/2
...
! ==========================================================================================================
   ...
   if (succes) then
      if (geometry%NbermSegments > 0) then
         call calculateGammaB (h, Hm0, z2, geometryFlatBerms, geometryFlatBerms%NbermSegments, &
                               gammaB, succes, errorMessage)
      else
         gammaB = 1.0d0
      endif
   endif
...
!!   @ingroup LibOvertopping
!***********************************************************************************************************
   subroutine checkInputdata (geometry, load, modelFactors, probContext, succes, errorMessage)
!***********************************************************************************************************
!
   implicit none
!
...
!
   ...
   character(len=10) :: par_txt  !< description model factor
...
!> convertModelfactorsRTO:
!! convert the model factors
!!   @ingroup LibOvertopping
!***********************************************************************************************************
   subroutine convertModelfactorsRTO(modelFactorsF, modelFactorsC)
!***********************************************************************************************************
!
   implicit none
   type (OvertoppingModelFactors), intent(in)  :: modelFactorsC !< model factors in C-like structure
   type (tpModelFactors),          intent(out) :: modelFactorsF !< model factors in Fortran like structure
! ==========================================================================================================

    modelFactorsF%fb      = modelFactorsC%factorDeterminationQ_b_f_b
    modelFactorsF%fn      = modelFactorsC%factorDeterminationQ_n_f_n
    modelFactorsF%fRunup1 = modelFactorsC%frunup1
    modelFactorsF%fRunup2 = modelFactorsC%frunup2
    modelFactorsF%fRunup3 = modelFactorsC%frunup3
    modelFactorsF%fS      = 10.0d0 ** modelFactorsC%fshallow
   end subroutine convertModelfactorsRTO

!> fillModelfactorsRTO:
!! fill the model factors
!!   @ingroup LibOvertopping
!***********************************************************************************************************
   subroutine fillModelfactorsRTO(modelFactors, fn, fb, frunup1, frunup2, frunup3, fshallow)
!***********************************************************************************************************
!
   implicit none
!
!  Input/output parameters
!
   real (kind=wp), intent(in)         :: fb              !< Model factor for breaking waves
   real (kind=wp), intent(in)         :: fn              !< Model factor for non-breaking waves
   real (kind=wp), intent(in)         :: frunup1         !< Model factor for wave run-up 1
   real (kind=wp), intent(in)         :: frunup2         !< Model factor for wave run-up 2
   real (kind=wp), intent(in)         :: frunup3         !< Model factor for wave run-up 3
   real (kind=wp), intent(in)         :: fshallow        !< Model factor for shallow waves
   type (tpModelFactors), intent(out) :: modelFactors    !< struct with model factors
! ==========================================================================================================

    modelFactors%fb      = fb
    modelFactors%fn      = fn
    modelFactors%fRunup1 = frunup1
    modelFactors%fRunup2 = frunup2
    modelFactors%fRunup3 = frunup3
    modelFactors%fS      = fshallow

   end subroutine fillModelfactorsRTO
