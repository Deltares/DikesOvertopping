...
! ==========================================================================================================
   ...
   if (succes) then
      ...
      if (succes) then
         ...
         ! calculate influence factor roughness
         gammaF = dot_product (horzLengths, rFactors) / sum(horzLengths)

         ! adjust influence factor if breaker parameter is greather than limit value
         if (succes) then
            if (ksi0 > ksi0Limit) then
               if (gammaB*ksi0Limit < 10.0d0) then
                  gammaF = gammaF + (1-gammaF)*gammaB*(ksi0-ksi0Limit)/(10-gammaB*ksi0Limit)
               endif
               gammaF = min(1.0d0, gammaF)
            endif
         endif
...
!!   @ingroup LibOvertopping
!***********************************************************************************************************
   subroutine calculateGammaB (h, Hm0, z2, geometry, NbermSegments, gammaB, succes, errorMessage)
!***********************************************************************************************************
!
   implicit none
!
...
!
   ...
   real(wp),               intent(in)  :: z2             !< 2% wave run-up (m)
   type(tpGeometry),       intent(in)  :: geometry       !< structure with geometry data
   integer,                intent(in)  :: NbermSegments  !< number of berm segments
...
!
   ...
   real(wp)  :: B (NbermSegments) !< berm widths (m)
   real(wp)  :: hB(NbermSegments) !< berm heights (m+NAP)
   real(wp)  :: LB(NbermSegments) !< berm influence lengths (m)
   real(wp)  :: rB(NbermSegments) !< relative berm widths
   real(wp)  :: dH(NbermSegments) !< berm depths (m)
   real(wp)  :: rD(NbermSegments) !< relative depths
   ...
   real(wp)  :: f1(NbermSegments) !< vector with help factors for combining influence factors
   real(wp)  :: f2(NbermSegments) !< vector with help factors for combining influence factors
...
! ==========================================================================================================
   ...
   succes = (succes .and. (N == NbermSegments))
