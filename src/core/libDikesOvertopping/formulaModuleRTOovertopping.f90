...
!***********************************************************************************************************
   subroutine calculateWaveRunup (Hm0, s0, ksi0, ksi0Limit, gammaB, gammaF, gammaBeta, modelFactors, z2, succes, errorMessage)
!***********************************************************************************************************
!
   implicit none
!
...
!  Local parameters
!
   real(wp)  :: z2_small ! 2% wave run-up for small breaker parameters (m)
   real(wp)  :: z2_large ! 2% wave run-up for large breaker parameters (m)
...
! ==========================================================================================================
   ...
   if (succes) then
      ...
      z2_small = Hm0 * modelFactors%fRunup1 * gammaB * gammaF * gammaBeta * ksi0

      ! calculate 2% wave run-up for large breaker parameters
      if (ksi0 > 0.0d0) then
         if (sqrt(ksi0) > 0.0d0) then
            z2_large = Hm0 * gammaF * gammaBeta * (modelFactors%fRunup2 - modelFactors%fRunup3/sqrt(ksi0))
            z2_large = max(z2_large, 0.0d0)
         else
            succes = .false.
         endif
      else
   ...
   ! calculate the 2% wave run-up
   if (succes) then

      ! compare breaker parameter with limit value
...
! ==========================================================================================================
   ...
   if (succes) then
      ...
      Qb = (0.067d0/sqrt(tanAlpha)) * gammaB * ksi0 * &
               exp( -modelFactors%fB * ((hCrest-h)/Hm0) * (1/(ksi0 * gammaBeta * gammaB * gammaF)) )
   
      ! non-breaking waves
      Qn = 0.2d0 * exp ( -modelFactors%fN * ((hCrest-h)/Hm0) * (1/(gammaBeta * gammaF)) )

      ! shallow waves
      Qs = modelFactors%fS * exp( -(hCrest-h) / (gammaBeta * gammaF * Hm0 * (0.33d0+0.022d0*max(ksi0,7.0d0))) )
...
!  Input/output parameters
!
   real(wp),         intent(in)  :: a, b, c, d     !< coefficients cubic function
...
!  Input/output parameters
!
   real(wp),        intent(in)  :: p, q  !< coefficients depressed cubic
