    ...
    do istep = 0, nstep
        ...
        if (varSlope > 0) then
            if (.not. extraSlopeTest) then
                write (ounit,'(a,f3.1,f8.3,f15.10)') '    1:', 1.0d0 / geometry%segmentSlopes(varSlope), &
                                                    load%h + overtopping%z2, overtopping%Qo
            else
                write (ounit,'(a,f5.1,f8.3,f15.10)') '    1:', 1.0d0 / geometry%segmentSlopes(varSlope), &
                                                    load%h + overtopping%z2, overtopping%Qo
            endif
        elseif ((crossSectionId == 2) .or. (crossSectionId == 3)) then
            write (ounit,'(f9.3,f8.3,f15.10)') geometry%yCoordinates(2), &
                                               load%h + overtopping%z2, overtopping%Qo
        elseif (crossSectionId == 4) then
